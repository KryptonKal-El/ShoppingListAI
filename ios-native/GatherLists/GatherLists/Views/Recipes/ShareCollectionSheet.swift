...
/// Sheet for managing collection sharing — add and remove collaborators.
struct ShareCollectionSheet: View {
    ...
    @State private var collaborators: [CollectionShare] = []
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Share \"\(collection.name)\"")
    ...
    private func collaboratorRow(share: CollectionShare) -> some View {
        HStack {
    ...
    private func loadCollaborators() async {
        isLoading = true
        do {
            collaborators = try await RecipeService.fetchCollectionShares(collectionId: collection.id)
        } catch {
            errorMessage = "Failed to load collaborators"
            print("[ShareCollectionSheet] Failed to load collaborators: \(error.localizedDescription)")
        }
        isLoading = false
    }
    ...
    private func addCollaborator() async {
        ...
        do {
            try await viewModel.shareCollection(id: collection.id, email: email)
            emailInput = ""
            await loadCollaborators()
            ...
            print("[ShareCollectionSheet] Failed to share: \(error.localizedDescription)")
        }
    ...
    private func removeCollaborator(email: String) async {
        ...
        do {
            ...
            print("[ShareCollectionSheet] Failed to unshare: \(error.localizedDescription)")
        }
